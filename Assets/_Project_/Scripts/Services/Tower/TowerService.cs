...
{
    [Inject] IPublisher<FigureActionMessage.FigurePlaced> _figurePlaced;
    [Inject] IPublisher<FigureActionMessage.FigureDisappear> _figureDisappear;
    [Inject] IPublisher<FigureActionMessage.TowerHeightLimit> _figureHeightLimit;
    ...
    [Inject]
    private void Construct(ISaveLoadProgressService saveLoadProgressService, ISpriteService spriteService, GameFactory gameFactory)
    {
    ...
    public void SetFigure(DraggingObject item, PointerEventData eventData, RectTransform rectTransform, Transform transform)
    {
        if (eventData.pointerDrag != null && eventData.pointerDrag.TryGetComponent(out FigureView figureView))
        {
            _transform = transform;

            if (StackedObjects.Count == 0)
                _maxHeight.Value = rectTransform.rect.height - (item.RectTransform.position.y + item.RectTransform.rect.height);
    ...
    private void PlaceOnTower(DraggingObject item)
    {
        ...
        if (StackedObjects.Count == 0)
        {
            ...
            item.transform.SetParent(_transform, worldPositionStays: true);
        ...
        {
            var lastBlock = StackedObjects[StackedObjects.Count - 1];
            Vector3 lastBlockLocalPosition = _transform.InverseTransformPoint(lastBlock.RectTransform.position);
            float xOffset = Random.Range(-blockHeight * 0.5f, blockHeight * 0.5f);
            newPosition = new Vector3(lastBlockLocalPosition.x + xOffset, lastBlockLocalPosition.y + blockHeight, 0);
            Vector3 worldPosition = _transform.TransformPoint(newPosition);
    ...
    {
        if (eventData.pointerDrag != null && eventData.pointerDrag.TryGetComponent(out DraggingObject draggingObject))
        {
    ...
    public void LoadTowerData(DraggingObject draggingObject, Transform transform, Canvas canvas)
    {
        ...
        foreach (var figureData in data.Figures)
        {
            ...
            newFigure.Image.sprite = _spriteService.GetSprite(figureData.SpriteName); 
